...
class AIStrategist:
    ...
    def _init_sqlite(self):
        ...
        # Graceful schema updates for version control, logging and provenance tracking
        for column_def in [
            ("created_at", "TEXT"),
            ("llm_model", "TEXT"),
            ("chunk_index", "INTEGER"),
            ...
            try:
                cursor.execute(f"ALTER TABLE strategy_insights ADD COLUMN {col_name} {col_type}")
        ...
        """)
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_tf_insight_id ON trade_feedback(insight_id)")
...
"""
            ...
            llm_model = getattr(self.llm_call, "model_name", "unknown-model")
            chunk_index = i
            source_chunk = chunk

            # 1. SQLite'ga saqlash with versioning and traceability columns
            with sqlite3.connect(self.db_path, timeout=30.0) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    """
                    INSERT INTO strategy_insights 
                    (id, source_id, insight_text, market_condition, setup_type, created_at, llm_model, chunk_index, source_chunk)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (insight_id, source_id, insight_text, condition, setup_type, created_at, llm_model, chunk_index, source_chunk),
                )
                conn.commit()
    ...
    def get_relevant_context(self, current_situation_desc: str, market_condition: str, limit: int = 3) -> str:
        ...
        """
        cache_key = f"{current_situation_desc}_{market_condition}_{limit}"
        ...
        if len(self._context_cache) > 1000:
            self._context_cache.clear()
        self._context_cache[cache_key] = result_text
        return result_text
