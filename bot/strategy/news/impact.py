...
def fetch_price_move(mt5_client, symbol, event_date_utc):
    
    start_dt = event_date_utc - timedelta(hours=24)
    end_dt = event_date_utc + timedelta(hours=6)
    
    rates_m5 = mt5_client.copy_rates_range(symbol, mt5_client.TIMEFRAME_M5, start_dt, end_dt)
...
def backfill_historical_data(mt5_client, csv_path: str, pair: str = "XAUUSD"):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            m5_pct, m1h_pct, m4h_pct, atr_mult = fetch_price_move(mt5_client, pair, dt_utc)
...
def analyze_historical_impact(currency_pair: str, event_name: str, lookback_months: int = 6, currency: str = None) -> dict:
    init_db()
    ...
    if currency:
        cursor.execute('''
            ...
            WHERE event_name = ? AND event_date >= ? AND currency = ?
        ''', (event_name, cutoff_dt.isoformat(), currency))
    else:
        cursor.execute('''
            SELECT * FROM historical_news 
