...
class TradingBot:
    ...
    def __init__(self, config: BotConfig):
        ...
        from bot.strategy.news_breakout_grid.grid_manager import NewsBreakoutGridManager
        self.grid_risk_guard = GridRiskGuard(
            max_daily_loss_pct=getattr(config, "news_breakout_grid_max_daily_loss_pct", 0.40),
            max_attempts_per_day=getattr(config, "news_breakout_grid_max_attempts_per_day", 15),
            initial_balance=1000.0  # Will be updated in start()
        )
        self.news_grid_manager = NewsBreakoutGridManager(config, self.grid_risk_guard)

        # Batch Processing holati
        self.current_symbol_index = 0

        # Profiler (High-resolution timing)
        self.profiler = LoopProfiler(enabled=True)
    ...
    def _fetch_data(self, symbol: str, timeframe: str, count: int) -> pd.DataFrame:
        ...
        # Anomal spayklarni (broker xatolarini) filtrlaymiz
        if not df.empty and 'close' in df.columns:
            median_close = df['close'].median()
            if median_close > 0:
                valid_mask = (df['high'] < median_close * 1.4) & (df['low'] > median_close * 0.6)
    ...
    def run_cycle(self, symbol: str) -> None:
        ...
        # 1. MA'LUMOT OLISH
        # ============================================================
        with self.profiler.track("1.1_fetch_major_data"):
        ...
        # ============================================================
        with self.profiler.track("1.2_regime_detection"):
        ...
        with self.profiler.track("2.1_portfolio_analysis"):
        ...
        with self.profiler.track("2.4_memory_and_shields"):
        ...
        if not df_major.empty:
            for _, row in df_major.tail(100).iterrows():
                try:
        ...
        with self.profiler.track("3.1_dl_predictor"):
        ...
        with self.profiler.track("3.4_rag_book_knowledge"):
            try:
        ...
        with self.profiler.track("3.5_ai_memory_and_weights"):
            try:
        ...
        # === AI Call Gate ===
        with self.profiler.track("3.6_ai_gate_check"):
            ...
            voting_signal = voting_result.get("signal", "HOLD")
            ...
            if voting_signal == "HOLD" or voting_score < min_conf:
                call_needed = False
                gate_reason = f"Kuchsiz signal: {voting_signal} (Ball: {voting_score}/{min_conf})"
            else:
        ...
        with self.profiler.track("3.8_shadow_collector_record"):
            try:
                indicators_data = {
                ...
                self.shadow_collector.record_state(
                    ...
                    ai_decision=final_decision,
                    recent_candles=df_major.to_dict('records') if not df_major.empty else None
                )
            except Exception as e:
    ...
    def start(self) -> None:
        ...
        # Initial balance for Grid Risk Guard
        acc_info = self.mt5.get_account_info()
        if acc_info:
            self.grid_risk_guard.initial_balance = acc_info.balance
        ...
        # Thread for News Breakout Grid if enabled
        if getattr(self.config, "news_breakout_grid_enabled", False):
            def news_grid_loop():
                import MetaTrader5 as mt5
                symbols = getattr(self.config, "news_breakout_grid_symbols", [])
                if not symbols: return
                logger.info(f"News Breakout Grid is active for: {symbols}")
                
                target_symbol = symbols[0] # Focus on the first whitelisted symbol
                self.news_grid_manager.activate(target_symbol)
                
                while self._running:
                    try:
                        if not self.news_grid_manager.is_active:
                            time.sleep(1)
                            continue
                        ...
                        if tick and sym_info:
                            # Current price we pass bid
                            self.news_grid_manager.tick(tick.bid, tick.ask, tick.bid, sym_info.point)
                            
                        time.sleep(0.01) # 10ms tick polling
                    except Exception as e:
                        logger.error(f"News Grid loop error: {e}")
                        time.sleep(1)
            threading.Thread(target=news_grid_loop, daemon=True).start()
        ...
        def periodic_retrain():
            ...
            while self._running:
                try:
                    ...
                    self.rl_agent.train_agent(total_timesteps=200000)
        ...
        try:
            while self._running:
                try:
                    ...
                    with self.profiler.track("0.1_cloud_settings_fetch"):
                        try:
                    ...
                    with self.profiler.track("0.3_news_trading_execution"):
                        try:
                    ...
                    # Har bir symbol uchun tahlil (Parallel)
                    with self.profiler.track("0.4_symbol_cycles_batch"):
                        import concurrent.futures
                        
                        batch_size = getattr(self.config, "batch_size", 3)
                        if batch_size > 1 and len(all_symbols) > 1:
                            max_workers = min(10, len(all_symbols))
                            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                                futures = {}
                                for symbol in all_symbols:
                                    if getattr(self.config, "news_breakout_grid_enabled", False) and symbol in getattr(self.config, "news_breakout_grid_symbols", []):
                                        logger.info(f"[{symbol}] Voting engine skipped (News Breakout Grid is active for this symbol).")
                                        continue
                                    futures[executor.submit(self.run_cycle, symbol)] = symbol
                                ...
                                for future in concurrent.futures.as_completed(futures):
                                    ...
                                    try:
                                        ...
                                        logger.error(f"[{symbol}] Parallel tahlilda xatolik: {e}")
                                    if not self._running:
                    ...
                    with self.profiler.track("0.7_ai_review_check"):
                        try:
                    ...
                    # Cloud sync
                    with self.profiler.track("0.8_cloud_sync_all"):
                        try:
                    ...
                    # Database backup
                    with self.profiler.track("0.9_db_backup_check"):
                        try:
                    ...
                    # End loop profiling & print summary report
                    cycle_duration = self.profiler.end_cycle()
                    self.profiler.log_summary()

                    # Garbage collection & memory cleanup at cycle end
                    import gc
                    gc.collect()
...
def run_cli() -> None:
    ...
    for noisy_logger in ["httpx", "httpcore", "urllib3", "supabase", "postgrest", "matplotlib", "asyncio"]:
