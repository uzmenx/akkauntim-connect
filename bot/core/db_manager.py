...
class DBManager:
    ...
    def _init_regime_tables(self):
        ...
        try:
            ...
            ''')
            self.execute('CREATE INDEX IF NOT EXISTS idx_regime_sym_tf ON regime_history(symbol, timeframe)')
            self.execute('CREATE INDEX IF NOT EXISTS idx_regime_ts ON regime_history(timestamp)')
