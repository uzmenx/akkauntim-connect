...
from news_impact_analyzer import analyze_historical_impact, get_cot_trend
...
def watch_upcoming_news(pair: str, minutes_ahead: int = 60) -> dict:
    ...
    hist_impact = analyze_historical_impact(pair, nearest_event['title'], lookback_months=6)
    ...
    if hist_impact and not hist_impact.get("insufficient_data"):
        ...
        if nearest_event['minutes_to_release'] <= 5 and best_confidence >= 0.7 and hist_impact['sample_size'] >= 10:
            if direction == "Bullish":
                recommended_action = "prepare_long"
            elif direction == "Bearish":
                recommended_action = "prepare_short"
            else:
                recommended_action = "reduce_position"
        elif nearest_event['minutes_to_release'] <= 5:
            recommended_action = "reduce_position"
    else:
        # If insufficient data
        if nearest_event['minutes_to_release'] <= 5:
            recommended_action = "reduce_position"
